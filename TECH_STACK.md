...
schema 10 在同一 SQLite 中保存一份派生索引：单条元信息、每 Memory 的处理状态及当前版本片段 BLOB；静态 sqlite-vec 0.1.9 精确距离计算。索引修订使用 UUID，只用于令在途旧任务失效，不保存索引代次。后台单写入者在事务外编码、短事务复核当前版本及修订号，首次固定 rowid 上界，随后按版本差异增量处理；单条增量失败不关闭已就绪索引。重试刷新修订，恢复备份和重建清空派生索引并重新扫描。
...
Embedding 直接编码搜索文本，不拼接任务指令或提供查询前缀配置；本地查询保留 Unicode 规范化和长度边界。在线连接测试只发送一次合成文本。Embedding 采用有界 `/embeddings` 请求，验证返回条数、顺序、有限数值和固定维度，再归一化。schema 14 只调整派生向量 BLOB 的维度约束，保留当前正文/历史和既有本地向量。唯一索引指纹包含端点、模型、实测维度和编码／分段版本；在途写入及查询重新核对指纹，旧任务不能使新配置使用旧向量。沿用单写入者、增量处理、暂停/重试及关键词降级，不引入第二套索引管理。
