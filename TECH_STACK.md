...
- 当前先用单次工具提议保留并整理已保存的 Memory、归并到已读候选或暂缓；第三阶段再升级有界取证循环，不创建第二条 Memory；
...
> 用户问题与有限会话上下文 → 生成有界检索变体 → Rust 核心统一搜索 → 读取当前 Memory 命中窗口 → 模型回答并引用来源 → 核心校验后呈现。第三阶段将取证部分升级为有界工具循环。
