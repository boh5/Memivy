...
正式应用使用 i18next + react-i18next，首批 `en` / `zh-CN` 静态内嵌资源；实现约定见 [I18N_PLAN.md](I18N_PLAN.md)。Rust 管理当前资料库独立的 `ui-preferences.json`，按系统偏好列表解析语言，通过带 revision 的快照和事件同步两个 WebView 与原生菜单。同一 WebView 的 React 主根和 Milkdown 工具栏共享 i18n 实例；切换只更新呈现，不重建编辑器或重启业务任务。

用户、模型和历史文本保留原文；新系统错误与整理原因保存稳定语义码，前端在渲染时翻译。每轮问答固定语言上下文，Agent 使用同一输出约定。回答完成后单独尝试概括简短会话标题，只发送有界的用户消息；成功后写入一次并更新两窗标题，不改变会话排序或草稿，失败留待后续成功回答再尝试。原生菜单使用同目录 JSON，系统权限说明通过专用 `InfoPlist.strings` 打包。实际检查与证据边界见 [DEVELOPMENT_PLAN.md](DEVELOPMENT_PLAN.md)。
