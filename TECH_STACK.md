...
- `model.rs` 共用有连接池、90 秒超时及 64 KB 响应上限的 HTTP 请求层，分别解析 JSON 与单个 Function Calling。内部新建/更新/暂缓参数由 `organization.rs` 转换、校验并提交现有 MemoryStore 事务；不扩展 MCP，不增加 Agent 循环。
