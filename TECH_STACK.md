...
正式讨论使用 `MemoryStore::start_turn` 持久化问题与等待状态，再按连接测试能力使用有界工具循环，或保留检索词提取和回答的基本流程。答案请求前冻结至多 8 份当前 Memory 版本节选，回答只允许引用这批依据；近期对话取最近 8 条消息、各至多 1000 字，作为讨论上下文。取消和重启恢复都以持久化状态阻止迟到完成。讨论输入及选定依据单独存为草稿，不加入记忆检索；结论必须经文字与目的地确认后调用 `save_conclusion`。正常正式配置缺失时可一次性沿用旧样机已有的本机模型配置，显式测试数据/配置覆盖不会读取用户旧配置。
...
- 支持多轮工具时最多两轮搜索/阅读，再提议保留、归并或暂缓；基本流程仍用单次工具提议。不创建第二条 Memory；
...
- 候选只来自统一搜索的当前 Memory，并应用项目条件，模型不能扫描全库；候选保留有效来源的项目元数据。原话明确标注项目时，筛选和事务提交都拒绝带有其他项目来源的自动续接；
...
- 初始候选最多 6 条当前版本，每条最多 3000 个字符的节选，受总证据预算约束；续接最多 3 处逐字匹配的局部修改，总修改范围不得超过旧正文一半。核心还检查目标仍是所用版本、原话仍有效且未被其他动作归属；未知目标、空替换、重叠范围及过时结果均拒绝执行。
...
> 用户问题与有限会话上下文 → 生成有界检索变体 → Rust 核心统一搜索 → 读取当前 Memory 命中窗口 → 模型回答并引用来源 → 核心校验后呈现。支持多轮工具时由模型在最多四轮内按需搜索、改查和补读，最后提交回答。

- 允许用同一个已配置模型提取或改写少量检索词；生成模型只处理问答；可选本地 Embedding 由统一搜索自行管理；
- 从具体记忆开始讨论时，重新绑定该 Memory 当前版本及已选命中窗口作为上下文；继续追问复用必要会话消息，按需检索相关内容；
- 限制候选数量、上下文长度与模型调用次数，不扫描或发送全库；最多 4 组字面变体、8 个当前正文来源及最近 8 条必要会话节选，正文受统一结果预算限制。原话和历史只供人工归档查看与恢复，不作为本轮新证据；
...
### 7.5 有界取证与能力降级（2026-09-10）

`model/tools.rs` 只管理有限工具协议和预算，`memory/agent.rs` 管理当前 Memory 证据、范围与版本。两种业务复用此循环；工具仅有 `search_memories`、`read_memory` 及本业务终结结果，不建立通用 Agent 框架。搜索仍只通过 `MemoryStore::search`，向量开关和 MCP 与此生成模型流程独立。

| AI 功能 | 执行方式 |
| --- | --- |
| 问一问、继续讨论、桌面提问、专题问答 | 最多四轮只读取证，再一次最终回答；专题范围固定 |
| 新录入后台整理 | 当前候选加最多两轮只读取证，再一次保留/归并/暂缓；核心事务一次应用 |
| 单篇正文整理、细化候选稿 | 当前正文或编辑草稿的固定生成流程，用户审核后保存 |
| 结论融合预览 | 用户选定正文及结论的固定生成流程，确认后保存 |
| 为 Memory 推荐专题 | 固定分类流程，最多三个候选，用户确认成员关系 |
| 为专题推荐 Memory | 固定取词、统一搜索及候选评估，最多八条，用户确认 |

每个任务总计 180 秒，预留 20 秒收尾；单次请求最多 90 秒、序列化 96 KiB，累计 384 KiB。任务最多八个已展示来源、12000 个 Unicode 字符和 48 KiB 正文；录入本条占用同一证据预算，超过 12000 字符保留可用 Memory 并明确整理未完成。重复且无新内容的阅读停止取证；首次未命中允许改写再查。协议保留完整 assistant 工具消息、call ID 和必须回传的字段，只读工具结果与终结结果不混用，越权/未知工具、截断、取消与过时版本不执行写入。

引用按当前版本及实际阅读范围登记。重叠区间合并；schema 11 的 `message_evidence_spans` 保存不连续范围，来源预览分别显示各段。取证过程中不反复覆盖引用表；最终一次绑定全部已展示片段，完成前再次验证所有依据。录入继续复用双方版本 CAS、草稿保护、回执和撤销，不在循环中修改记忆。

设置的“保存并测试连接”依次用合成内容测试结构化 JSON、单次工具与完整工具回传。能力缓存以配置指纹绑定，保存在独立私有 `model-capabilities.json`；更换端点/模型/凭证后失效，不保存凭证明文。未测试配置沿用基本流程；缺多轮能力时问答保持固定 RAG、录入保持单次工具；明确缺单次工具时暂停自动整理。网络/鉴权失败不缓存成不支持，设置显示具体能力及降级。正文整理等固定功能不因此升级为 Agent。
...
用户、模型和历史文本保留原文；新系统错误与整理原因保存稳定语义码，前端在渲染时翻译。每轮问答固定语言上下文，Agent 与基本路径共用输出约定；本轮生成的标题随回答持久化。原生菜单使用同目录 JSON，系统权限说明通过专用 `InfoPlist.strings` 打包。实际检查与证据边界见 [DEVELOPMENT_PLAN.md](DEVELOPMENT_PLAN.md)。
