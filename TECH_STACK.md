...
设置默认关闭，下载并启用 Qwen3-Embedding-0.6B Q8_0（639150592 字节、1024 维）。固定 HF repo/commit/SHA-256 描述，权重统一进入正式数据目录 `models/huggingface/hub/` 的 blobs/snapshots；断点校验后原子发布，完整 blob 的发布中断直接补快照。启用意图写入独立 `embedding.json`，不依赖生成模型配置。
