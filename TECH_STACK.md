...
`model/stream.rs` 负责 OpenAI-compatible SSE 传输，保留原始 assistant 工具消息及必要回传字段；参数完整、JSON 校验通过后才分发工具。自然文本逐段交给宿主，不把思考或残缺参数展示为正文。使用现有 reqwest，不增加 SDK 或 Provider 平台。

`memory/agent.rs` 提供一个有界循环和共享读取／写入合同：搜索、目录分页、按稳定 ID 分段读当前／历史／来源、回读会话、`write_memory`。搜索统一通过 `MemoryStore::search`，普通搜索仅当前活动正文；目录没有正文证据身份。讨论额外提供本轮记录／暂停选项与整次撤销工具，无强制答案终结工具。
...
`set_turn_options` 由同一 Agent 声明本轮需要“仅确认”还是“回答／讨论”（`reply_kind`），与是否写记忆、回答长短分别处理；未声明时进入回答路径。仅确认的成功输入沿用快捷窗收起和不生成追问的行为。暂停／恢复仍是独立的维护状态，不因选择回答而自动恢复。没有前置分类模型、关键词分类器或额外输入模式。
...
设置及首次缺少缓存时，使用合成内容验证结构化 JSON、流式文本、单次工具和完整多轮回传。完整 Agent 必须通过流式文本与多轮工具；能力缓存绑定配置指纹，不保存凭证明文。不支持时停止 Agent，保留本地编辑、关键词搜索和显式 MCP capture；不切换模型或降级至固定 RAG。网络／鉴权失败与明确不支持分别处理。
