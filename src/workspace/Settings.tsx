...
export default function SettingsPanel({onClose,onRestore,onChanged,initialPage="general"}:{initialPage?:Page;onClose:()=>void;onRestore:(id:string)=>void;onChanged:()=>void}){
 ...
 const sub=page in modelNames?t(modelHints[page as Kind]):t(page==='ai'?'page.aiSubtitle':(page==='general'||page==='desktop')?'page.generalSubtitle':page==='data'?'page.dataSubtitle':'page.mcpSubtitle');
 return <Modal title={t('title')} className="model-settings" onClose={()=>{if(!locked)onClose()}}><div className="settings-shell"><aside className="settings-nav"><h2><img src={logo} alt=""/>{t('title')}</h2><nav aria-label={t('nav.categoryLabel')}>{tabs.map(([id,icon,label])=><button key={id} className={active===id?'active':''} aria-current={active===id?'page':undefined} disabled={locked} onClick={()=>navigate(id)}><Icon name={icon}/>{t(label)}</button>)}</nav><p>{t('nav.tagline')}<br/><small>Memivy</small></p></aside><main className="settings-main"><header className="model-page-header">{(page in modelNames)&&<button className="icon-button" aria-label={t('actions.backToModels')} disabled={locked} onClick={()=>navigate('ai')}><Icon name="back"/></button>}<div><h2>{title}</h2><p>{sub}</p></div></header>
 {!native&&<p className="model-preview-note">{t('preview.readOnly')}</p>}
 {page==='ai'&&<><div className="settings-page"><div className="model-capabilities">{(Object.keys(modelNames) as Kind[]).map(k=>{const b=models?.[k],enabled=k==='llm'?!!b:k==='embedding'?!!(embedding?.enabled||embedding?.preparing||embedding?.paused):!!voice?.enabled;const status=k==='embedding'?indexLabel(embedding):k==='voice'?(voice?.session?.error?t('voice.status.transcriptionFailed'):voice?.error?t('voice.status.needsCheck'):enabled?t('status.enabled'):t('status.disabled')):enabled?t('status.configured'):t('status.notConfigured');const local=b?.source==='local';return <section key={k} className="model-capability"><div className="model-cap-icon"><Icon name={k==='llm'?'spark':k==='embedding'?'search':'mic'} size={23}/></div><div className="model-cap-body"><h3>{t(modelNames[k])}{enabled&&<span className="model-badge">{t(local?'model.local':'model.service')}</span>}</h3><p>{t(modelHints[k])}</p><small>{enabled?(local?k==='embedding'?'Qwen3-Embedding · 0.6B':'Qwen3-ASR · 0.6B':b?.model):k==='llm'?t('model.connectOwn'):t('model.recommendedLocal',{size:k==='embedding'?'640 MB':'1.02 GB'})}</small></div><div className="model-cap-side"><small>{models?status:t('status.readingSettings')}</small><button className="model-text-button" disabled={!models||locked} onClick={()=>navigate(k)}>{t(enabled?'actions.manage':'actions.configure')} →</button></div></section>})}</div><p className="model-subtle"><Icon name="lock" size={14}/>{t('model.offlineNotice')}</p></div><footer className="model-footer"><span>{t('model.connectionLocalOnly')}</span></footer></>}
 ...
 {(page==='general'||page==='desktop')&&<div className="settings-page general-settings-page"><LanguageSettings/><DesktopSettings focusEntry={page==='desktop'}><VoiceSettings shortcutOnly/></DesktopSettings></div>}
