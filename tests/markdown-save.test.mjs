...
async function editMemory(t, version='v-a') {
  ...
  const detail={key:f.keyA,state:'active',title:"Title",body:"## Original body",current:{id:version,capture_ids:[]},history:[],sources:[]};
  ...
  await f.settle();
  f.find(page,n=>n.props['aria-label']==="Edit body").props.onClick();await f.settle();
...
test('rich-editor shortcut cannot bypass a version conflict',async t=>{
  ...
  f.find(editor,n=>n.type==='MarkdownEditor').props.onChange("Preserve my draft");await f.settle();
  ...
  assert.equal(f.db.get('memory:a').body,"Preserve my draft");
});
...
test('title and body edits do not replace the editor helper while persistence is pending',async t=>{
 ...
 for(const change of [()=>f.find(editor,n=>n.props['aria-label']==="Edit memory title").props.onChange({target:{value:"New title"}}),()=>f.find(editor,n=>n.type==='MarkdownEditor').props.onChange("New body")]){
