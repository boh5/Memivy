...
fn schema_15(root: &std::path::Path) -> Connection {
    let db = Connection::open(root.join("memivy.db")).unwrap();
    for migration in [
        ...
        include_str!("../../../migrations/memory/009_current_memory.sql"),
        include_str!("../../../migrations/memory/current_fts.sql"),
        include_str!("../../../migrations/memory/010_embedding.sql"),
        include_str!("../../../migrations/memory/011_agent_evidence.sql"),
        include_str!("../../../migrations/memory/012_library_revision.sql"),
        include_str!("../../../migrations/memory/013_ui_changes.sql"),
        include_str!("../../../migrations/memory/014_model_vectors.sql"),
        include_str!("../../../migrations/memory/015_organization_reason.sql"),
    ] {
...
#[test]
fn schema16_upgrade_keeps_existing_titles_and_drafts_without_bulk_naming() {
    let root = tempfile::tempdir().unwrap();
    let db = schema_15(root.path());
    ...
    let first = id();
    let second = id();
    for (conversation, title, draft) in [
        (&first, "New discussion", "尚未发送的原话"),
        (&second, "以前创建的会话标题", ""),
    ] {
        db.execute("INSERT INTO conversations(id,title,draft,created_at,updated_at) VALUES(?1,?2,?3,123,456)", rusqlite::params![conversation,title,draft]).unwrap();
    }
    let changes: i64 = db
        .query_row("SELECT count(*) FROM ui_changes", [], |r| r.get(0))
        .unwrap();
    drop(db);
    let store = MemoryStore::open(root.path()).unwrap();
    assert_eq!(store.conversation(&first).unwrap().title, "New discussion");
    assert_eq!(store.conversation(&first).unwrap().draft, "尚未发送的原话");
    assert_eq!(
        store.conversation(&second).unwrap().title,
        "以前创建的会话标题"
    );
    assert_eq!(store.conversation(&second).unwrap().updated_at, 456);
    assert_eq!(
        count(
            &store,
            "SELECT count(*) FROM conversations WHERE title_generated=0"
        ),
        2
    );
    assert_eq!(count(&store, "SELECT count(*) FROM ui_changes"), changes);
    assert_eq!(count(&store, "SELECT count(*) FROM messages"), 0);
    store.check_integrity().unwrap();
}

#[test]
fn migration_preserves_each_unsent_draft_and_both_reviewed_text_variants() {
    let root = tempfile::tempdir().unwrap();
    let db = schema_15(root.path());
    let source = SourceRef::Version(id());
    for (key, body) in [
        ("capture", "记一半的想法"),
        ("question", "问一半的问题"),
        ("quick_capture", "快捷来源的原话"),
        ("quick_question", "快捷问题的草稿"),
    ] {
        let payload = json!({"key":key,"request_id":id(),"title":"","body":body,"expected_version":null,"context":[source],"origin":{"kind":"user","app":"Browser","project":"工作","uri":"https://example.test"}});
        db.execute(
    ...
    let old_save = format!("conclusion:{}", id());
    let destination = Destination::Existing {
        memory_id: id(),
        expected_version: id(),
    };
    let payload = json!({"key":old_save,"request_id":id(),"title":"结论","body":"用户选择的原始结论","expected_version":null,"context":[],"conclusion":{"destination":destination,"merged_body":"用户还在编辑的整合正文"}});
    db.execute(
        "INSERT INTO workspace_drafts(key,payload) VALUES(?1,?2)",
        rusqlite::params![old_save, payload.to_string()],
    )
    .unwrap();
    drop(db);
    let store = MemoryStore::open(root.path()).unwrap();
    store.check_integrity().unwrap();
    assert_eq!(
        store.workspace_draft("input").unwrap().unwrap().body,
        "记一半的想法"
    );
    let quick = store.workspace_draft("quick_input").unwrap().unwrap();
    assert_eq!(quick.body, "快捷来源的原话");
    assert_eq!(quick.context, vec![source]);
    assert!(matches!(quick.origin,Some(Origin::User{app,..}) if app=="Browser"));
    let saved = store
        .workspace_draft(&old_save.replacen("conclusion:", "save:", 1))
        .unwrap()
        .unwrap();
    assert_eq!(saved.body, "用户选择的原始结论");
    assert_eq!(saved.destination, Some(destination));
    let mut recovered = Vec::new();
    for conversation in store.conversations(100).unwrap() {
        assert_eq!(conversation.title, "未发送的草稿");
        assert!(store.messages(&conversation.id, 0, 100).unwrap().is_empty());
        recovered.push(
            store
                .workspace_draft(&format!("discussion:{}", conversation.id))
                .unwrap()
                .unwrap()
                .body,
        );
    ...
    let mut expected = vec![
        "问一半的问题".to_string(),
        "快捷问题的草稿".to_string(),
        "用户还在编辑的整合正文".to_string(),
    ];
    expected.sort();
    assert_eq!(recovered, expected);
    assert_eq!(
        count(&store, "SELECT count(*) FROM memories"),
        0,
        "migration never submits or invents memories"
    );
}
...
#[test]
fn migrated_whole_document_review_is_an_editor_draft_not_append_text() {
    for already_editing in [false, true] {
        let root = tempfile::tempdir().unwrap();
        let db = schema_15(root.path());
        let memory = id();
        let version = id();
        let old_save = format!("conclusion:{}", id());
        db.execute(
            "INSERT INTO memories(id,created_at,updated_at) VALUES(?1,1,1)",
            [&memory],
        )
        .unwrap();
        db.execute("INSERT INTO memory_versions(id,memory_id,title,body,actor,reason,created_at) VALUES(?1,?2,'计划','原正文','user','create',1)",rusqlite::params![version,memory]).unwrap();
        db.execute(
            "UPDATE memories SET current_version_id=?2 WHERE id=?1",
            rusqlite::params![memory, version],
        )
        .unwrap();
        let key = format!("memory:{memory}");
        if already_editing {
            let draft = json!({"key":key,"request_id":id(),"title":"计划","body":"当前编辑中的独立草稿","expected_version":version,"context":[]});
            db.execute(
                "INSERT INTO workspace_drafts(key,payload) VALUES(?1,?2)",
                rusqlite::params![key, draft.to_string()],
            )
            .unwrap();
        }
        let payload = json!({"key":old_save,"request_id":id(),"title":"计划","body":"选择的补充内容","expected_version":null,"context":[],"conclusion":{"destination":{"kind":"existing","memory_id":memory,"expected_version":version},"merged_body":"原正文与补充整合后的全文"}});
        db.execute(
            "INSERT INTO workspace_drafts(key,payload) VALUES(?1,?2)",
            rusqlite::params![old_save, payload.to_string()],
        )
        ...
        let editor = store.workspace_draft(&key).unwrap().unwrap();
        assert_eq!(editor.expected_version.as_deref(), Some(version.as_str()));
        if already_editing {
            assert_eq!(editor.body, "当前编辑中的独立草稿");
            let recovered = store.conversations(100).unwrap();
            assert_eq!(recovered.len(), 1);
            assert_eq!(recovered[0].draft, "原正文与补充整合后的全文");
        } else {
            assert_eq!(editor.body, "原正文与补充整合后的全文");
            assert!(store.conversations(100).unwrap().is_empty());
        }
        assert_eq!(store.memory(&memory).unwrap().current.body, "原正文");
    }
}
...
#[test]
fn schema15_failed_inputs_retry_the_new_agent_without_duplicate_user_messages() {
    for state in ["failed", "cancelled", "interrupted"] {
        ...
        let user = id();
        let assistant = id();
        let original = "  以前留下的想法 🙂\n";
        db.execute(
            "INSERT INTO conversations(id,title,created_at,updated_at) VALUES(?1,'旧讨论',1,1)",
            [&conversation],
        )
        .unwrap();
        db.execute(
            "INSERT INTO turns(id,conversation_id,fingerprint) VALUES(?1,?2,X'00')",
            rusqlite::params![input, conversation],
        )
        .unwrap();
        db.execute("INSERT INTO messages(id,turn_id,conversation_id,role,text,status,created_at) VALUES(?1,?2,?3,'user',?4,'complete',44)",rusqlite::params![user,input,conversation,original]).unwrap();
        db.execute("INSERT INTO messages(id,turn_id,conversation_id,role,text,status,created_at,answer) VALUES(?1,?2,?3,'assistant','旧的未完成正文',?4,45,'{broken old answer protocol')",rusqlite::params![assistant,input,conversation,state]).unwrap();
        drop(db);
        let store = MemoryStore::open(root.path()).unwrap();
        let before = store.turn(&input).unwrap();
        assert_eq!(before.user.text, original);
        assert_eq!(before.assistant.text, "旧的未完成正文");
        let resumed = store.retry_agent_input(&input, &id()).unwrap();
        assert_eq!(resumed.user_message_id, user);
        assert_eq!(resumed.input_text, original);
        assert!(resumed.protocol.is_empty());
        write(&store, &resumed, Destination::New, "以前留下的想法");
        store
            .finish_agent_input(&input, &resumed.attempt_id, true, &[])
            .unwrap();
        let messages = store.messages(&conversation, 0, 100).unwrap();
        assert_eq!(messages.len(), 2);
        assert_eq!(messages.iter().filter(|m| m.role == "user").count(), 1);
        let receipt = &store.agent_input_receipts(&input).unwrap()[0];
        assert_eq!(
            store
                .capture_by_id(receipt.capture_id.as_ref().unwrap())
                .unwrap()
